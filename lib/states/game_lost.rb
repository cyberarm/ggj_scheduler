class SchedulerGame
  class States
    class GameLost < CyberarmEngine::GuiState
      def setup
        ...
        flow(width: 1.0, height: 1.0) do
          ...
          stack width: 0.7, height: 1.0, padding: 24 do
            ...
            banner "Failed!", width: 1.0
            title "Took: #{format_clock}", width: 1.0

            zone = @map.zones.detect { |z| z.type == :entry_door }
            title "Cleared: #{@map.travellers.size - zone.occupancy} of #{@map.travellers.size} Travellers", width: 1.0
          end
        end
      end
